...
/// dominant-7, minor-7, major-7, and diminished-7 chords as a single <b>movable shape</b> on the A/D/G strings (alphaTab
...
/// (Major7), <c>-1</c> = diminished 7th (Diminished7, the ♭♭7). <c>R</c> is taken so the 3rd never needs a
/// negative fret. Bb7/Eb7/F7 fall out as
...
public sealed class BeginnerShellStrategy : IVoicingStrategy
{
    ...
    public Voicing Voice(Chord chord)
    {
        ...
        // Major7 = maj3 + maj7, Diminished7 = min3 + dim7 (the ♭♭7, a semitone below the minor 7th → −1).
        (int thirdOffset, int seventhOffset) = chord.Quality switch
        {
            Quality.Dominant7 => (-1, 0),
            Quality.Minor7 => (-2, 0),
            Quality.Major7 => (-1, 1),
            Quality.Diminished7 => (-2, -1),
            _ => throw new NotSupportedException(
                $"The MVP shell shape covers Dominant7, Minor7, Major7, and Diminished7 only; got {chord.Quality}."),
        };
