...
| `RenderOptions` / `VoicingStrategy` | **score-render-component thread.** `RenderOptions(bool ShowChordNames, bool ShowChordDiagrams, VoicingStrategy Voicing)` — the render-time presentation bag carried from the bridge's `renderOptions` (generate/entityPreview/loadExercise). `VoicingStrategy` ships only `ByDifficulty` (the existing `VoicingBook.Lookup(chord, difficulty)` selection); other values fail loud (CAGED-shape preference deferred to `caged-system`/`voicings`). Player-kind toggles (metronome/count-in) never reach Core — they're alphaTab-API-only in the JS component. |
