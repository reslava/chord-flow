...
ChordFlow.sln
  src/
    ...
    ChordFlow.Desktop/    net10.0-windows  — the host. The ONLY project with a UI package.
      ...
      wwwroot/            index.html (Practice ⇄ Content ⇄ Scales views), bridge.js (shared transport), playback-component.js (shared alphaTab api + transport engine, composing the PlaybackClock time clock) + player-controls-component.js (shared transport controls: play/stop/tempo/soundfont/metronome/count-in) + harmony-controls-component.js (HarmonyControlsR — the shared exercise-definition strip: harmony/key/feel/comping+vol/lead+vol/difficulty/voicing window/Generate/Save/Practiced), score-render-component.js (shared alphaTex→alphaTab render component — incl. the opt-in `debugPanel` alphaTex scratchpad), app.js (the ONE-page Practice shell: Score ⇄ Sheet view toggle over one engine), content-crud.js (Content editor) + fretboard-render-component.js (shared SVG fretboard render component) + fretboard-sandbox.html (hand-fed dev harness for it), scales.js (Scales: interval-set → lit neck), chord-sheet-render-component.js (ChordSheetR — pure-SVG chord sheets) + chord-sheets.js (ChordFlowSheetView — the Sheet VIEW of the Practice page), alphaTab, font/, soundfont/
  tests/
...
**The chord-sheet render component (`chord-sheet-render-component.js`, `window.ChordFlowChordSheet`)** — ChordSheetR — draws a Core `ChordSheet` model as **one self-contained `<svg>`** (leadsheet **Layout A** or grid **Layout B**), the whole sheet — chord tokens, tone strips, compact fret diagrams, `%` similes — in a single node so the *same* SVG serves both the on-screen body and export (screen == export). It is a **dumb drawer** (C1): all notations/tones/diagrams are resolved in Core; JS notation/adornment/layout/theme toggles are pure re-renders (no round-trip, C3). It reuses FretR's **diagram model + function palette**, not FretR's DOM component (a deliberate reversal of the original "embed FretR" design, so the sheet stays one SVG under the no-external-libs CSP — see `chord-sheets-maker` design §Implementation-note). Its **Sheet-view module** `chord-sheets.js` (`window.ChordFlowSheetView` — the Sheet view of the ONE Practice page since harmony-controls-r) owns only what is sheet-specific: the display strip (Layout / Chords / + line / Below cell / Tone labels / Theme / Marker mode) + the exports, the ChordSheetR mount, and the playback marker. The sheet has **no bridge verb of its own anymore**: the model + `cellSchedule` ride the unified **`loadScore`** reply as projections of the generate/loadExercise pass (`ExerciseRendering.RenderWithSheet` → `ChordSheetBuilder`; comping resolved **unconditionally**, so the model always carries tone + diagram data and Below-cell adornments are pure display toggles — never a re-request). The shell fans the one page engine's beat signal into `sheetView.onBeat` (both markers track even while their view is hidden, so the mid-playback Score ⇄ Sheet toggle is seamless). **Retired:** `chordSheet` / `chordSheetResult` / `chordSheetError` and `ChordSheetHandler` (the builder stays as the projection). Still sheet-specific: **`exportChordSheet`** → the host prints the print-styled light page via WebView2 `PrintToPdfAsync` (SVG/PNG export are client-side; no PDF library, C4), replied by `chordSheetPdfDone`.
...
UI picks harmony (Song/Progression) + comping + optional lead + params (key/tempo/difficulty/feel)
  ...
  → loadScore envelope (C#→JS) — ONE reply, every projection of the pass (`ExerciseRendering.RenderWithSheet`): the tex, the chord **schedule** (one ChordChange per chord change: 0-based bar/beat + a FretboardDiagram of the comped voicing — the now/next feed), and the **chord-sheet projection** (`sheet` + `cellSchedule` for the Sheet view + its marker). All derive from the same RealizedSong + CompingPlan + render, so the tab, the sheet, its marker, and the now/next boards cannot drift
  → app.js: api.tex(tex)
...
┌──────────────────────────────────────────────────────────────────┐
│ UI (JS) — dumb views                                               │
│   playback-component           (alphaTab api + transport + bus)    │
│   player-controls-component    (shared transport controls UI)      │
│   score-render-component       (notation; built on playback-comp.) │
│   chord-sheet-render-component (chord sheet; marker over playback) │
│   fretboard-render-component   (guitar spatial SVG)                │
└──────────────────────────────────────────────────────────────────┘
```

**Playback engine seam (`playback-component.js`, `window.ChordFlowPlayback`).** alphaTab fuses rendering and playback in one `AlphaTabApi(surface, settings)`, so the api + transport + audio + soundfont round-trip + beat/state events live in a shared **`ChordFlowPlayback`** engine. The engine composes **`PlaybackClock`** — the time-based playback clock (`playback/metronome-true-marker`): it maps `playerPositionChanged`'s continuous tick to `(bar, quarterBeat)` against the **playback tick timeline** (`api.tickCache.masterBars` — NOT the score model's `masterBar.start`, which zero-widths a pickup bar on v1.8.3), dedupes to one step per quarter (`ticksPerStep` create-opt, default 960 = quarter), and emits the **`"position"`** bus event — the time-linear signal metronome-shaped consumers follow (the Sheet marker's Visual-metronome mode today; a future metronome widget subscribes the same way, zero new wiring). The event-driven `"beat"` (from `activeBeatsChanged`; its beat number is the rendered note/rest index, not a time position) stays the signal for everything genuinely event-shaped: Per-chord marker, Now/Next, the `beatChanged` bridge echo. `score-render-component.js` (**ScoreR**) *composes* one — becoming a **thinner, notation-only layer** — and reaches through `engine.getApi()` for its render/notation concerns (staff, notation-display options, staff-display profile, key/feel/tempo pickers, debug panel); its public handle is unchanged, so the Practice player (`app.js`) and the Content-preview (`content-crud.js`) consume ScoreR exactly as before. A headless consumer (e.g. the Chord Sheets page) mounts a `ChordFlowPlayback` with its surface off-screen to get transport + `onBeat` without any notation chrome.

**Shared transport controls (`player-controls-component.js`, `window.ChordFlowPlayerControls`) — PlayerControlsR** (`ui/player-controls-component`). The player transport — play/pause · stop · tempo · soundfont · **metronome** · **count-in** · an optional **Now/Next** toggle — is one shared widget that **binds to a `ChordFlowPlayback` handle** (it does *not* own the engine). Since harmony-controls-r the Practice page mounts it **once, at page level** — the shell's transport strip (the Score ⇄ Sheet toggle at its head, then PlayerControlsR bound to ScoreR's engine via `handle.getEngine()`; ScoreR is created with `transport:false` there) — so the one transport survives the view toggle. Other score surfaces (the Content preview) keep the default in-strip mount ScoreR does itself. This ended the duplication that had let a control silently break on one surface (the `syncToggle` drop that killed metronome/count-in — `playback/metronome-countin-fix`): the transport now lives and is fixed in one place, and every control handler is wrapped so a throw surfaces in the console instead of being swallowed by the DOM dispatcher. To let the controls react to the engine without each surface re-forwarding events, **`ChordFlowPlayback` is a small multi-subscriber event source** — `engine.on("beat" | "position" | "stateChange" | "ready" | "finished" | "soundFontsListed", cb)` (the create-time `onBeat`/… callbacks are sugar seeding the same buses); PlayerControlsR self-subscribes to `ready`/`stateChange`/`soundFontsListed`, while the surface subscribes to `beat`/`finished` for its own cursor/marker.

```
  Practice page (app.js — ONE page, harmony-controls-r)
  ├── HarmonyControlsR    harmony · key · feel · comping+RhythmVol · lead+LeadVol ·
  │                       difficulty · voicing frets · Generate/Save/Practiced
  ├── transport strip     [Score ⇄ Sheet toggle] + PlayerControlsR
  │                         play · stop · tempo · soundfont · metronome · count-in · Now/Next
  ├── Now/Next boards     ChordFlowNowNext — view-independent
  ├── view surfaces       ┌ Score view: ScoreR (staff toggles + debug panel + alphaTab)
  │   (collapse-swap)     └ Sheet view: ChordFlowSheetView (sheet strip + ChordSheetR SVG + exports)
  └── library pane        saved exercises — page-level, both views
        │
        │  PlayerControlsR + HarmonyControlsR volumes bind to the ONE engine (ScoreR.getEngine());
        │  the shell fans engine beats (+ the clock's "position" steps) out to the score cursor, Now/Next, and the sheet marker
        ▼
  ┌────────────────────────────────────────────────────────────────────────────────┐
  │ ChordFlowPlayback   — ONE for the whole Practice page (mid-playback view toggle) │
  │   alphaTab api + transport + audio + soundfont + multi-subscriber event bus      │
  │   composes PlaybackClock (tick → quarter steps — the time-linear "position" feed) │
  │   emits:  beat · position · stateChange · ready · finished · soundFontsListed    │
  └────────────────────────────────────────────────────────────────────────────────┘
  (other score surfaces — the Content preview — still compose their own engine via ScoreR)
```

**Chord-sheet playback.** The sheet's playback payload rides the unified `loadScore` reply (`sheet` + `cellSchedule` — projections of the same pass as the tex; the old `chordSheetResult` carrier is retired). `cellSchedule` maps score `(bar,beat)` → the sounding `ChordSheetR` cell/chord. The `ChordSheetR` SVG is addressable (`<g data-section data-row data-cell>` cells + nested `<g data-chord>`), with `highlight()`/`highlightBeat()`/`clearHighlight()` driving a screen-only marker in two selectable modes — **Visual metronome** (per-beat column, default — follows the engine's time-based `"position"` clock, so it steps evenly through notes, sustains and silences alike) and **Per chord** (active chord segment — event-driven `"beat"`, chord onsets *are* events) — both over the same bar wash (export stays inert). Sheet, cellSchedule and tex come from one realized-song pass, so the marker tracks the audio beat-for-beat — even across a mid-playback view toggle.
