...
ChordFlow.sln
  src/
    ...
    ChordFlow.Desktop/    net10.0-windows  — the host. The ONLY project with a UI package.
      ...
      wwwroot/            index.html (Practice ⇄ Content ⇄ Scales views), bridge.js (shared transport), playback-component.js (shared alphaTab api + transport engine) + player-controls-component.js (shared transport controls: play/stop/tempo/soundfont/metronome/count-in), score-render-component.js (shared alphaTex→alphaTab render/transport component — incl. the opt-in `debugPanel` alphaTex scratchpad), app.js (Practice), content-crud.js (Content editor) + fretboard-render-component.js (shared SVG fretboard render component) + fretboard-sandbox.html (hand-fed dev harness for it), scales.js (Scales: interval-set → lit neck), chord-sheet-render-component.js (ChordSheetR — pure-SVG chord sheets) + chord-sheets.js (the Chord Sheets page shell), alphaTab, font/, soundfont/
  tests/
...
UI picks harmony (Song/Progression) + comping + optional lead + params (key/tempo/difficulty/feel)
  ...
  → loadScore envelope (C#→JS) — also carries a chord **schedule** (one ChordChange per chord change: 0-based bar/beat + a FretboardDiagram of the comped voicing), produced by the render pass (`RenderResult.Schedule`) so it can't drift from the tab; ChordFlowNowNext drives the now/next fretboards off it
  → the **Chord Sheets path shares this exact schedule**: `ChordSheetHandler` runs the same `AlphaTexRenderer.Render` and surfaces `RenderResult.Schedule` as `chordSheetResult.chordSchedule`, so the now/next boards render on both surfaces off one Core producer (no separate `ChordScheduleBuilder`)
  → app.js: api.tex(tex)
...
```
  ...
  └────────────────────────────────────────────────────────────────────────────────┘
        │  calls    engine.play / stop / setTempo / setMetronome / setCountIn / setSoundFont
        │  listens  engine.on( ready | stateChange | soundFontsListed )
        ▼
  ┌────────────────────────────────────────────────────────────────────────────────┐
  │ ChordFlowPlayback   — ONE per surface (no cross-page transport)                  │
  │   alphaTab api + transport + audio + soundfont + multi-subscriber event bus      │
  │   emits:  beat · stateChange · ready · finished · soundFontsListed               │
  └────────────────────────────────────────────────────────────────────────────────┘
        ▲
        │  the surface itself listens engine.on( beat | finished ) for its cursor/marker,
        │  and owns its drawing (ScoreR: getApi() staff · ChordSheetR: the SVG)
```
